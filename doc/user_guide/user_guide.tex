...
Each enumeration package also contains a nested package named \texttt{C} which holds a mirror of \texttt{E} suitable for passing to a binded C or C++ function. See Section \ref{C Interfacing}. C compilers represent an \texttt{enum} as an \texttt{int}, while Ada sizes \texttt{E} to fit its literals, so \texttt{E} cannot be passed across the binding directly. The mirror type, \texttt{E\_C}, is declared with \texttt{Convention => C}, which gives it the size of a C \texttt{int}, and carries the same literal names and values as \texttt{E}. \\
