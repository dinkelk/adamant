...
package body Variable_Serializer with SPARK_Mode => On is
   ...
   -- The bytes of a value. SPARK accepts the conversion since both types have the
   -- same compile time size.
   function Bytes_Of is new Ada.Unchecked_Conversion (Source => T, Target => Byte_Array);
   ...
   begin
      ...
      if Stat /= Success then
         ...
         declare
            ...
            Bytes : constant Byte_Array := Bytes_Of (Src);
         begin
            -- Copy the serialized bytes to the destination:
            Dest (Dest'First .. Dest'First + Num_Bytes_Serialized - 1) := Bytes (0 .. Num_Bytes_Serialized - 1);
         end;
         return Stat;
      else
         -- Nothing to copy:
