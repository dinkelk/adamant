...
private
   ...
   -- T1: Verify parameter validation rejects N_Filter values causing derivative filter instability (N*dt >= 2.0).
   overriding procedure Test_Derivative_Filter_Stability (Self : in out Instance);
   -- T2: Verify parameter validation rejects inverted integral limits (I_Min_Limit > I_Max_Limit).
