...
package body Component.Ccsds_Product_Extractor.Implementation is
   ...
   -- in the model. Publishing here -- after all components are initialized but
   -- before the rate groups begin ticking -- guarantees a valid value exists in
   -- the data product database for downstream consumers to fetch before the first
   -- packet arrives. Products without a default builder are skipped, so a model
   -- that declares no defaults makes this a no-op (legacy behavior).
   overriding procedure Set_Up (Self : in out Instance) is
   begin
      if Self.Extraction_List /= null then
         declare
         ...
         begin
            for Apid_Entry of Self.Extraction_List.all loop
               if Apid_Entry.Extract_List /= null then
