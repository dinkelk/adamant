...
private
   ...
   -- This unit test fills the queue and drops multiple messages in succession, verifying the event count matches the number of drops.
   overriding procedure Test_Multiple_Drops (Self : in out Instance);
   -- This unit test exercises Dispatch_N to verify correct FIFO ordering under incremental partial dequeue.
   overriding procedure Test_Partial_Dispatch (Self : in out Instance);
