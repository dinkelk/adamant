...
private
   ...
   -- This test fills both copies with garbage and checks that no copy is valid, that a
   -- restore reports No_Valid_Copy, and that the restored data is zeroed.
   ...
   -- is stored, and that each restore returns the newest data.
   ...
   -- This test checks that a copy saved with a different layout id or a different
   -- data length is treated as invalid.
   ...
   -- restore, the layout id, and the No_Valid_Copy behavior.
