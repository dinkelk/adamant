...
private
   ...
   -- This test fills both regions with garbage, zeros, and ones and checks that Restore
   -- and Init report No_Valid_Copy with a zero value, that Init leaves the regions
   ...
   -- This test emulates a reboot at each point during a save and checks that Restore
   ...
   -- This test checks that a save before any Init writes copy A, and that the saves
   -- then alternate.
   overriding procedure Test_Save_Before_Init (Self : in out Instance);
   -- This test instantiates the store on a larger packed record and checks that save
   -- and Restore round trip every field and that the CRC covers all of them.
