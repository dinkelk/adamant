...
private
   ...
   subtype Entry_Bytes is Basic_Types.Byte_Array (0 .. T'Object_Size / Basic_Types.Byte'Object_Size - 1);

   -- The serializer overlays byte arrays by address, which is outside the
   -- SPARK memory model, so it is instantiated in a nested package that is
   -- not analyzed and reached through the two wrappers below. SPARK treats
   -- the wrappers as trusted boundary operations.
