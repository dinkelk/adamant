...
package body Variable_Database with SPARK_Mode => On is
   ...
   -- The bodies of the serializer wrappers are not analyzed by SPARK since
   -- the serializer they call is outside the SPARK memory model:
   function Store (Data : out Entry_Bytes; Value : in T) return Serializer_Types.Serialization_Status with SPARK_Mode => Off is
   begin
      return Serial.T_Serializer.To_Byte_Array (Data, Value);
   end Store;

   function Load (Value : out T; Data : in Entry_Bytes) return Serializer_Types.Serialization_Status with SPARK_Mode => Off is
   begin
      return Serial.T_Serializer.From_Byte_Array (Value, Data);
   end Load;
